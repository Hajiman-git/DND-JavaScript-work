...
-- ==============================================
-- ДОБАВЛЕНИЕ НОВЫХ ПОЛЕЙ И ТАБЛИЦ ДЛЯ ИНФОПУЗЫРЯ
-- ==============================================
...
-- ==============================================
-- РАСШИРЕННЫЕ ДАННЫЕ ДЛЯ ИНФОРМАЦИОННОГО ПУЗЫРЯ
-- ==============================================
...
-- 2. НАЗНАЧЕНИЕ ТЕГОВ ДЛЯ НОВЫХ ТОВАРОВ
-- Игровые теги
INSERT INTO product_tags (product_id, tag_id)
SELECT p.id, t.id 
...
-- 4. ДОБАВЛЕНИЕ ИСТОРИИ ДЕЙСТВИЙ ДЛЯ ДЕМОНСТРАЦИИ
-- Для пользователя gamer_pro (любит игровые товары)
DO $$
DECLARE
    gamer_id INTEGER;
    product_rec RECORD;
...
-- Для пользователя office_worker (любит офисные и бюджетные товары)
DO $$
DECLARE
    office_id INTEGER;
BEGIN
...
-- Для пользователя apple_fan (любит Apple)
DO $$
DECLARE
    apple_id INTEGER;
BEGIN
...
-- Для пользователя budget_user (любит бюджетные товары)
DO $$
DECLARE
    budget_id INTEGER;
BEGIN
